...
let { nodeFactory } = requireBox(
  "dapp-services/services/dapp-services-node/generic-dapp-service-node"
);
const {
  deserialize,
  encodeName,
  ...
  resolveProviderData,
  getLinkedAccount,
} = requireBox("dapp-services/services/dapp-services-node/common");
...
const saveToIPFS = async (data) => {
  ...
  let cachedValue = cache.get(uri);
  if (
    (cachedValue && !process.env.IPFS_POSTGRESQL_FORCE_REPLAY) ||
    (cachedValue &&
      process.env.IPFS_POSTGRESQL_FORCE_REPLAY &&
      process.env.IPFS_POSTGRESQL_FORCE_REPLAY.toString() !== "true")
  ) {
    logger.warn(`already cached ${uri}`);
    return uri;
  }

  let filesAdded = await ipfs.files.add(bufData, {
    "raw-leaves": true,
    "cid-version": 1,
    "cid-base": "base58btc",
    pin: true,
  });
  const theHash = filesAdded[0].hash;

  if (
    (process.env.IPFS_POSTGRESQL &&
      process.env.IPFS_POSTGRESQL.toString() === "true") ||
    (args.ipfsPostgres && args.ipfsPostgres.toString() === "true")
  ) {
    await fetch(
      `http://localhost:${
        process.env.IPFS_POSTGRESQL_PORT || 8090
      }/add?key=${theHash}`,
      {
        method: "POST",
        headers: {
  ...
  if (resUri != uri) throw new Error(`uris mismatch ${resUri} != ${uri}`);
  ...
  try {
    ...
    if (
      ...
      res = await Promise.race([
        // add option to get file from go postgresql
        fetch(
          `http://localhost:${
            process.env.IPFS_POSTGRESQL_PORT || 8090
          }/get?key=${fileName}`
...
const readPointer = async (hashWithPrefix, contract, sidechain) => {
  let hash = hashWithPrefix.toString("hex").slice(8);
  let matchHash = hash;
  if (matchHash.includes(","))
    matchHash = Buffer.from(matchHash).toString("hex");
  if (nodeosLatest.toString() === "true") {
    matchHash = matchHash
      .match(/.{16}/g)
      .map((a) => a.match(/.{2}/g).reverse().join(""))
      .join("");
  } else {
    matchHash = matchHash
      .match(/.{16}/g)
  ...
  let result = res.rows.find((a) => {
    let myHash = hashData256(Buffer.from(a.data, "hex"));
    return myHash == hash;
  });
...
const calculateBucketShard = (
  ...
  let num = new BigNumber(hashedKey, 16);
  // logger.debug(`fullkey ${fullkey} num ${num.toString()}`);
  let bucket = num.mod(buckets).toNumber();
  let shard = num.idiv(Math.pow(2, 6)).mod(shards).toNumber();
  return { shard, bucket };
};
...
const fetchShardUri = async (contract, table, shard, sidechain) => {
  try {
    ...
    if (!res.rows.length)
      throw new Error(
        `Error fetching shard ${shard} of table ${table} of account ${contract}`
      );
...
const stringToName = (str, keytype = null, keysize = 64) => {
  ...
  if (isUpperCase(str) || keytype === "symbol") {
...
const fetchBucket = async (bucketPointer, contract, sidechain, keysize) => {
  let bucketRawData = await readPointer(bucketPointer, contract, sidechain);
  return parseBucket(bucketRawData, keysize);
};

const extractRowFromBucket = async (
  bucketData,
  scope,
  key,
  contract,
  ...
  if (rowPointer) {
    return await readPointer(rowPointer, contract, sidechain);
  }
};

const extractRowFromShardData = async (
  shardData,
  bucket,
  scope,
  key,
  ...
  const bucketData = await fetchBucket(
    bucketPointer,
    contract,
    sidechain,
    keysize
  ...
  return await extractRowFromBucket(
    bucketData,
    scope,
    key,
    contract,
...
const getRowRaw = async (
  ...
  const { shard, bucket } = calculateBucketShard(
    key,
    keytype,
    keysize,
    scope,
  ...
  let innerkey = stringToNameInner(
    stringToName(key, keytype, keysize),
    keysize
  ).toString();
  ...
  return await extractRowFromShardData(
    shardData,
    bucket,
    innerscope,
    innerkey,
...
const getEosRpc = (sidechain) =>
  sidechain && sidechain.nodeos_endpoint
    ? new JsonRpc(sidechain.nodeos_endpoint, { fetch })
    : rpc;
...
const getVconfig = async (account, table, sidechain) => {
  ...
  if (!abi.tables.find((table) => table.name == ".vconfig")) {
    logger.warn(
      `account ${account} missing .vconfig from abi, may be warmup issues`
    );
  ...
  if (!vconfig) throw new Error(".vconfig table not initialized");
...
const getRequiredIpfsData = async (
  ...
  let uris = [];
  logger.info(
    `getting required ipfs data for ${account} ${table} ${scope} ${key}`
  );
  const { shards, buckets_per_shard } = await getVconfig(
    account,
    table,
    sidechain
  );
  ...
  // logger.info(refKey)
  // logger.info(scope)
  const { bucket, shard } = calculateBucketShard(
    refKey,
    keyType,
    keysize,
    scope,
  ...
  const formattedScope = stringToNameInner(
    stringToName(scope, "number")
  ).toString();
  const formattedKey = stringToNameInner(
    stringToName(refKey, keyType, keysize),
    keysize
  ).toString();
  ...
  if (tableEntryUri) {
    ...
    const tableEntryRawData = await readPointer(
      tableEntryUri,
      account,
      sidechain
    );
...
async function verifyIPFSConnection() {
  try {
    ...
    logger.error(
      `ipfs connection failed (${e}), IPFS_HOST = ${
...
const readRemoteIPFS = async (contract, uri, sidechain) => {
  ...
  if (sidechain)
    mainnet_account = await getLinkedAccount(
      null,
      null,
      contract,
      sidechain.name
  ...
  for (let i = 0; i < remoteProviders.length; i++) {
    ...
    const packageid = await resolveProviderPackage(
      mainnet_account,
      service,
      provider
    );
    ...
    const result = await postData(
      `${remote.endpoint}/v1/dsp/${service}/get_uri`,
      { uri }
    );
...
const getSidechain = async (chain, sidechain) => {
  ...
  if (!sidechainObj)
    throw new Error(`Unsupported sidechain ${chain} requested from contract`);
  return sidechainObj;
};
...
nodeFactory("ipfs", {
  commit: async ({ rollback, replay, exception }, { data }) => {
    ...
    if (process.env.PASSIVE_DSP_NODE == "true" || replay || exception) {
      logger.warn(
        `commit PASSIVE_DSP_NODE ${process.env.PASSIVE_DSP_NODE} || replay: ${replay} || exception ${exception}`
      );
      return;
    }
    return {
  ...
  warmupcode: async ({ event, rollback }, { uri, code }) => {
    if (rollback) {
      ...
      logger.info(
        `WARMUP CODE: getting uri for account ${event.payer} : ${
          code || event.payer
        } - ${uri}`
      );
      let data;
  ...
  warmuprow: async (
    { event, rollback },
    { uri, code, table, scope, index_position, key, keysize }
  ) => {
    if (rollback) {
      // rollback warmup
      ...
      try {
        ...
        logger.info(
          `WARMUP ROW: getting data for account ${event.payer} : ${
            code || event.payer
          } - ${table} ${scope} ${index_position} ${key} ${keysize}`
        );
        keysize = keysize * 8; // sizeof(PrimKey) measures each 8 bits as size 1
        const requiredData = await getRequiredIpfsData(
          uri,
          code || event.payer,
          table,
          scope,
  ...
  warmupchain: async (
    { event, rollback },
    { shard, code, table, chain, scope, index_position, key, keysize }
  ) => {
    if (rollback) {
      // rollback warmup
      ...
      try {
        ...
        logger.info(
          `WARMUP CHAIN: getting data for account ${event.payer} : ${
            code || event.payer
          } - ${
            sidechain ? sidechain.name : "mainnet"
          } ${shard} ${table} ${scope} ${index_position} ${key} ${keysize}`
        );
        keysize = keysize * 8; // sizeof(PrimKey) measures each 8 bits as size 1
        const requiredData = await getRequiredIpfsData(
          null,
  ...
  cleanchain: async (
    { rollback, exception, replay },
    { shard, code, table, chain, uris }
  ) => {
    if (rollback || exception || replay) {
      return;
    }
  ...
  api: {
    get_table_row: async ({ body }, res) => {
      try {
        ...
        const row = await deserializeRow(
          contract,
          table,
          rowRaw,
          body.sidechain,
