...
void dgoods::mint(name to,
    ...
    check(config_table.exists(), "Symbol table does not exist, setconfig first");
    auto config_singleton  = config_table.get();
    auto dgood_id =  config_singleton.dgood_available_primary_key;
    ...
    config_singleton.dgood_available_primary_key++;
    config_table.set( config_singleton, get_self() );
}
    ...
    void apply (uint64_t receiver, uint64_t code, uint64_t action ) {
        ...
        if ( code == self ) {
            switch( action ) {
                ...
                EOSIO_DISPATCH_HELPER( dgoods, DAPPSERVICE_ACTIONS_COMMANDS() )
                EOSIO_DISPATCH_HELPER( dgoods, (xsignal) )
            }
        }
