...
function MT_ARR_LEAF_ROW_OFFSET(d) {
  return (1n << d) - 1n;
}
function MT_ARR_FULL_TREE_OFFSET(d) {
  return (1n << (d + 1n)) - 1n;
}
function MT_NUM_LEAVES(d) {
  return 1n << d;
}
...
module.exports = async ({ proto, address }) => {
  ...
  await zeos().then(async (instance) => {
    ...
    var note_commitments_ptr = instance.allocate(
      note_commitments,
    ...
    var indices = [];
    var idx =
      MT_ARR_LEAF_ROW_OFFSET(tree_depth) +
      (leaf_count % MT_NUM_LEAVES(tree_depth));
    var tos =
      (leaf_count / MT_NUM_LEAVES(tree_depth)) /*=tree_idx*/ *
      MT_ARR_FULL_TREE_OFFSET(tree_depth);
    ...
    for (const i of indices) {
      res = await fetch(endpoint + "/v1/chain/get_table_rows", {
        ...
        body: JSON.stringify({
          ...
          key_type: "uint64_t",
          lower_bound: i.toString(),
          upper_bound: i.toString(),
        }),
      });
