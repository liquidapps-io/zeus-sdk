...
const { deserialize, eosDSPGateway, generateABI, genNode, paccount, forwardEvent, resolveProviderData, resolveProvider, getProviders, resolveProviderPackage, paccountPermission } = requireBox('dapp-services/services/dapp-services-node/common');
...
module.exports = async ({ proto, address }) => {
  ...
  let idx = 0;
  var wasmCode = addressParts[idx++];
  var method = addressParts[idx++];
  var payload = addressParts[idx++];
  var wasm = await loadWasmForContract(wasmCode);
  let res;
  let data;
