...
// File: contracts/ERC11555Tradeable.sol
...
contract ERC1155Tradable is ContextMixin, ERC1155, NativeMetaTransaction, Ownable {
  ...
  function burn(
    ...
    if(tokenSupply[id] == value) {
      if (bytes(ERC1155(address(this)).uri(id)).length > 0) {
          delete customUri[id];
      }

      delete creators[id];
      delete tokenSupply[id];
    }
  }
