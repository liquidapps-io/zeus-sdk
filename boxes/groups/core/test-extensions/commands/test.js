...
const startLocalEnvCommand = requireBox(
  "localenv-extensions/commands/start-localenv"
);
...
module.exports = {
  ...
  builder: (yargs) => {
    yargs
      ...
      .option("backend", {
        describe:
          'backend API to stream trxs, firehose || state_history_plugin || ""',
      ...
      .option("kill-services", {
        describe:
          "kill all current running nodes and services after running tests",
        default: true,
      })
      .option("legacy-cdt", {
      ...
      .option("ipfs-postgres", {
        describe: "enable to use postgresql to fetch/add IPFS files",
        default: false,
      })
  ...
  handler: async (args) => {
    ...
    try {
      ...
      const contractCommand = `${
        args.contract ? subContractCommand : "-- test"
      }`;
      ...
      try {
        if (args.killServices === true) {
          console.log(
            emojMap.ok + "shutting down nodeos and all other services"
          );
