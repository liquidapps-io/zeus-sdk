...
function copyFileSync( source, target, simulate , overwrite , newIgnoreList, rootPath) {
    ...
    if ( fs.existsSync( target ) ) {
        if ( fs.lstatSync( target ).isDirectory() ) {
            ...
            if(!sourceBuf.equals(targetBuf) && !overwrite){
...
function copyFolderRecursiveSync( source, target, simulate, ignoreList , overwrite, newIgnoreList, rootPath) {
    var files = [];
    ...
    if ( stats.isDirectory() ) {
        ...
        files.forEach( function ( file ) {
            ...
            if (fstats.isDirectory() ) {
                copyFolderRecursiveSync( curSource, curTarget , simulate, ignoreList , overwrite, newIgnoreList, rootPath);
            } else {
                copyFileSync( curSource, curTarget, simulate ,overwrite , newIgnoreList, rootPath);
            }
        } );
    }
}
...
const handler = async (args, globalCopyList=[])=>{
        ...
        if(global.currentzeusBoxJson){
            ...
            if(!currentzeusBoxJson.dependencies)
                currentzeusBoxJson.dependencies = [];
            ...
            if(args.createDir){
                ...
                if(fs.existsSync(newDir))
                    throw new Error("path already exists", newDir);
                fs.mkdirSync(newDir);
        ...
        if(currentzeusBoxJson.dependencies.indexOf(args.box) != -1 && (!args.update || global.updated.indexOf(args.box) != -1))
            return;
        
        global.updated.push(args.box);
        if(!args.no_sample){
        ...
        // stdout = await execPromise(`${process.env.UNZIP || 'unzip'} ${inputPath} -d ${extractPath}`,{});
        ...
        if(zeusBoxJson.dependencies){
            if(zeusBoxJson.dependencies.filter(pkg=>currentzeusBoxJson.dependencies.indexOf(pkg) == -1).length)
                console.log(emojMap.gear + "Unboxing:",zeusBoxJson.dependencies.filter(pkg=>currentzeusBoxJson.dependencies.indexOf(pkg) == -1).map(a=>a.yellow).join(','));
        ...
        if(zeusBoxJson.ignore)
            ignoreList = zeusBoxJson.ignore.map(a=>path.join(extractPath,a));
        // compare if any file already exists
        // console.log('copying staged files');
        
        copyFolderRecursiveSync( extractPath, path.resolve('.'), false , ignoreList, args.update, newIgnoreList, path.resolve('.'));
        globalCopyList.forEach(a=>{
            if(newIgnoreList.indexOf(a) == -1)
        ...
        try{
            ...
            if(args.test && !args.no_sample){
                console.log(emojMap.zap + "testing");
        ...
        if(args.no_sample || !zeusBoxJson.commands || Object.keys(zeusBoxJson.commands).length == 0)
            return;
...
module.exports = {
    ...
    builder: (yargs) => {
        yargs.option('create-dir', {
                ...
                // describe: '',
                default: false
        }).alias('c', 'create-dir').alias('t', 'test').alias('u', 'update').example('$0 unbox -u events');
    },
    command: 'unbox <box>',
    handler
}
