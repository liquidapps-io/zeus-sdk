...
if (fs.existsSync(__dirname + '/.env')) { dotenv.config({ path: __dirname + '/.env' }); }

const modifyEnv = (key, value) => {
  let envConfig = {};
  if (fs.existsSync(__dirname + '/.env')) { envConfig = dotenv.parse(fs.readFileSync(__dirname + '/.env')); }
  envConfig[key] = value;
...
const _getParam = (name, description, defaultValue, options) => {
  ...
  if (readFromUser == '') { return defaultValue; }
};
