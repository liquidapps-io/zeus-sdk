...
const {
  ...
  getBoxesDir,
  requireBox,
} = require("@liquidapps/box-utils");
const { loadModels } = requireBox("seed-models/tools/models");
const disabledServices = ["log", "history"];
var configPath =
  process.env.DSP_CONFIG_FILE || path.join(os.homedir(), ".dsp", "config.toml");
if (!fs.existsSync(configPath))
  throw new Error(
    `Config file missing. please copy sample-config.toml to ${configPath}`
  );
...
function createSidechainModels(data) {
  const liquidxMappingsDir = path.resolve(
    getBoxesDir(),
    `liquidx/models/liquidx-mappings/`
  );
  const localSidechainsDir = path.resolve(
    getBoxesDir(),
    `liquidx/models/eosio-chains/`
  );
...
if (globalEnv.FIREHOSE_ENABLE && !globalEnv.FIREHOSE_GRPC_ADDRESS)
  throw new Error(
    "FIREHOSE_GRPC_ADDRESS, FIREHOSE_GRPC_PORT and FIREHOSE_GRPC_SECURED is required if FIREHOSE_ENABLE true"
  );
...
const DSP_ALLOW_API_NON_BROADCAST =
  globalEnv.DSP_ALLOW_API_NON_BROADCAST || false;
const DSP_CRON_SCHEDULE_RETRY_ON_FAILURE =
  globalEnv.DSP_CRON_SCHEDULE_RETRY_ON_FAILURE || false;
const DSP_MAX_REQUEST_RETRIES = globalEnv.DSP_MAX_REQUEST_RETRIES || 10;
const DSP_EOSIO_TRANSACTION_EXPIRATION =
  Number(globalEnv.DSP_EOSIO_TRANSACTION_EXPIRATION) || 180;
...
const DSP_AWS_KMS_SECRET_ACCESS_KEY =
  globalEnv.DSP_AWS_KMS_SECRET_ACCESS_KEY || "";
...
const IPFS_POSTGRESQL = globalEnv.IPFS_POSTGRESQL || false;
const IPFS_POSTGRESQL_PORT = globalEnv.IPFS_POSTGRESQL_PORT || 8090;
const IPFS_POSTGRESQL_FORCE_REPLAY =
  globalEnv.IPFS_POSTGRESQL_FORCE_REPLAY || 8090;
...
const NODEOS_MANDEL_RETRY_IRREVERSIBLE =
  globalEnv.NODEOS_MANDEL_RETRY_IRREVERSIBLE || false;

// Liquidstorage
const DSP_LIQUIDSTORAGE_UPLOAD_LIMIT =
  globalEnv.DSP_LIQUIDSTORAGE_UPLOAD_LIMIT || "10mb";
...
const DEMUX_BYPASS_DATABASE_HEAD_BLOCK =
  globalEnv.DEMUX_BYPASS_DATABASE_HEAD_BLOCK || false;
const DEMUX_MAX_PENDING_MESSAGES = globalEnv.DEMUX_MAX_PENDING_MESSAGES || 5000;
const DEMUX_PROCESS_BLOCK_CHECKPOINT =
  globalEnv.DEMUX_PROCESS_BLOCK_CHECKPOINT || 1000;
...
const NODEOS_CHAINID =
  globalEnv.NODEOS_CHAINID ||
...
const DSP_BROADCAST_EVENT_TIMEOUT =
  globalEnv.DSP_BROADCAST_EVENT_TIMEOUT || 10000;
...
const EVM_ETHEREUM_MAX_PRIORITY_FEE_PER_GAS =
  globalEnv.EVM_ETHEREUM_MAX_PRIORITY_FEE_PER_GAS || "500000";
const EVM_ETHEREUM_MAX_FEE_PER_GAS =
  globalEnv.EVM_ETHEREUM_MAX_FEE_PER_GAS || "500000";
const EVM_ETHEREUM_KEYS_PER_CONSUMER =
  globalEnv.EVM_ETHEREUM_KEYS_PER_CONSUMER || "";
const EVM_ETHEREUM_GAS_PRICE_MULT =
  Number(globalEnv.EVM_ETHEREUM_GAS_PRICE_MULT) || 1.2;
...
const EVM_ROPSTEN_MAX_PRIORITY_FEE_PER_GAS =
  globalEnv.EVM_ROPSTEN_MAX_PRIORITY_FEE_PER_GAS || "";
const EVM_ROPSTEN_MAX_FEE_PER_GAS = globalEnv.EVM_ROPSTEN_MAX_FEE_PER_GAS || "";
const EVM_ROPSTEN_KEYS_PER_CONSUMER =
  globalEnv.EVM_ROPSTEN_KEYS_PER_CONSUMER || "";
const EVM_ROPSTEN_GAS_PRICE_MULT =
  Number(globalEnv.EVM_ROPSTEN_GAS_PRICE_MULT) || 1.2;
...
const EVM_RINKEBY_MAX_PRIORITY_FEE_PER_GAS =
  globalEnv.EVM_RINKEBY_MAX_PRIORITY_FEE_PER_GAS || "";
const EVM_RINKEBY_MAX_FEE_PER_GAS = globalEnv.EVM_RINKEBY_MAX_FEE_PER_GAS || "";
const EVM_RINKEBY_KEYS_PER_CONSUMER =
  globalEnv.EVM_RINKEBY_KEYS_PER_CONSUMER || "";
const EVM_RINKEBY_GAS_PRICE_MULT =
  Number(globalEnv.EVM_RINKEBY_GAS_PRICE_MULT) || 1.2;
...
const EVM_MATIC_MAX_PRIORITY_FEE_PER_GAS =
  globalEnv.EVM_MATIC_MAX_PRIORITY_FEE_PER_GAS || "";
...
const EVM_MATIC_GAS_PRICE_MULT =
  Number(globalEnv.EVM_MATIC_GAS_PRICE_MULT) || 1.2;
...
const EVM_MUMBAI_MAX_PRIORITY_FEE_PER_GAS =
  globalEnv.EVM_MUMBAI_MAX_PRIORITY_FEE_PER_GAS || "";
const EVM_MUMBAI_MAX_FEE_PER_GAS = globalEnv.EVM_MUMBAI_MAX_FEE_PER_GAS || "";
const EVM_MUMBAI_KEYS_PER_CONSUMER =
  globalEnv.EVM_MUMBAI_KEYS_PER_CONSUMER || "";
const EVM_MUMBAI_GAS_PRICE_MULT =
  Number(globalEnv.EVM_MUMBAI_GAS_PRICE_MULT) || 1.2;
...
const EVM_BINANCE_MAX_PRIORITY_FEE_PER_GAS =
  globalEnv.EVM_BINANCE_MAX_PRIORITY_FEE_PER_GAS || "";
const EVM_BINANCE_MAX_FEE_PER_GAS = globalEnv.EVM_BINANCE_MAX_FEE_PER_GAS || "";
const EVM_BINANCE_KEYS_PER_CONSUMER =
  globalEnv.EVM_BINANCE_KEYS_PER_CONSUMER || "";
const EVM_BINANCE_GAS_PRICE_MULT =
  Number(globalEnv.EVM_BINANCE_GAS_PRICE_MULT) || 1.2;
...
const EVM_BSCTEST_MAX_PRIORITY_FEE_PER_GAS =
  globalEnv.EVM_BSCTEST_MAX_PRIORITY_FEE_PER_GAS || "";
const EVM_BSCTEST_MAX_FEE_PER_GAS = globalEnv.EVM_BSCTEST_MAX_FEE_PER_GAS || "";
const EVM_BSCTEST_KEYS_PER_CONSUMER =
  globalEnv.EVM_BSCTEST_KEYS_PER_CONSUMER || "";
const EVM_BSCTEST_GAS_PRICE_MULT =
  Number(globalEnv.EVM_BSCTEST_GAS_PRICE_MULT) || 1.2;
const EVM_TRANSACTION_POLLING_TIMEOUT =
  Number(globalEnv.EVM_TRANSACTION_POLLING_TIMEOUT) || 750;
...
const EVM_MAX_PRIORITY_FEE_PER_GAS =
  globalEnv.EVM_MAX_PRIORITY_FEE_PER_GAS || "";
...
// Assert .env
if (["state_history_plugin"].indexOf(DEMUX_BACKEND) === -1)
  throw new Error("DEMUX_BACKEND must be 'state_history_plugin'");
if (["http", "https"].indexOf(IPFS_PROTOCOL) === -1)
  throw new Error("IPFS_PROTOCOL must be either 'http' or 'https'");
if ([true, false].indexOf(NODEOS_SECURED) === -1)
  throw new Error("NODEOS_SECURED must be either true or false");
if (NODEOS_MANDEL_RETRY_BLOCKS > 0 && NODEOS_MANDEL_RETRY_IRREVERSIBLE)
  throw new Error(
    "NODEOS_MANDEL_RETRY_BLOCKS cannot be used with NODEOS_MANDEL_RETRY_IRREVERSIBLE"
  );
...
const serviceNames = loadModels("dapp-services")
  .map((file) => file.name)
  .filter((s) => !disabledServices.includes(s));

const DSP_SERVICES_ENABLED =
  globalEnv.DSP_SERVICES_ENABLED || serviceNames.join(",");
const services = DSP_SERVICES_ENABLED.split(",");

if (
  services.includes("link") &&
  !globalEnv.EVM_PRIVATE_KEY &&
  !globalEnv.EVM_ETHEREUM_PRIVATE_KEY &&
  !globalEnv.EVM_BINANCE_PRIVATE_KEY
)
  throw new Error("EVM_PRIVATE_KEY is required to run link service");
if (
  services.includes("link") &&
  !globalEnv.EVM_ENDPOINT &&
  !globalEnv.EVM_ETHEREUM_ENDPOINT &&
  !globalEnv.EVM_BINANCE_ENDPOINT
)
  throw new Error("EVM_ENDPOINT is required to run link service");
...
let commonEnv = {
  ...
  IPFS_PROTOCOL,
  IPFS_POSTGRESQL,
  IPFS_POSTGRESQL_PORT,
  IPFS_POSTGRESQL_FORCE_REPLAY,
  DSP_ACCOUNT,
...
if (ORACLES_PREFIXES) {
  ORACLES_PREFIXES.split("|").forEach((item) => {
    ...
    Object.assign(commonEnv, {
      [`ORACLE_PREFIX_${item.payer.toUpperCase()}_${item.name.toUpperCase()}`]:
        item.fullAddress,
    });
  });
}
...
const createDSPSidechainServices = (sidechain) => {
  ...
  commonEnv = {
    ...commonEnv,
    [`DSP_PRIVATE_KEY_${sidechain.name.toUpperCase()}`]:
      sidechain.dsp_private_key,
    [`FIREHOSE_GRPC_ADDRESS_${sidechain.name.toUpperCase()}`]:
      sidechain.firehose_grpc_address,
    [`FIREHOSE_GRPC_SECURED_${sidechain.name.toUpperCase()}`]:
      sidechain.firehose_grpc_secured,
    [`FIREHOSE_GRPC_PORT_${sidechain.name.toUpperCase()}`]:
      sidechain.firehose_grpc_port,
  };
  const sidechainCommonEnv = {
    ...
    NODEOS_MANDEL_RETRY_IRREVERSIBLE:
      sidechain.NODEOS_MANDEL_RETRY_IRREVERSIBLE || false,
  ...
  return [
    {
      ...
      script: path.join(
        getBoxesDir(),
        getBoxName(`services/dapp-services-node/index.js`),
        "services",
        "dapp-services-node",
        "index.js"
      ),
      ...
      : {
          ...
          env: {
            ...
            DEMUX_BYPASS_DATABASE_HEAD_BLOCK:
              sidechain.demux_bypass_database_head_block || false,
            DEMUX_MAX_PENDING_MESSAGES:
              sidechain.demux_max_pending_messages || 5000,
            DEMUX_PROCESS_BLOCK_CHECKPOINT:
              sidechain.demux_process_block_checkpoint || 1000,
...
const createDSPServiceApp = (name) => {
  return {
    ...
    script: path.join(
      getBoxesDir(),
      getBoxName(`services/${name}-dapp-service-node/index.js`),
      "services",
      `${name}-dapp-service-node`,
      "index.js"
    ),
    autorestart: true,
    ...clusterParams,
...
if (FIREHOSE_ENABLE) {
  ...
  apps.push({
    ...
    node_args: [`--max_old_space_size=${DEMUX_MAX_MEMORY_MB}`],
  });
}
